 ...
 *   routeRedirects: [{name, fromPattern, to, maxPerSession, pauseMediaSelector?}]
...
(function () {
  ...
  // A full-screen player (YouTube Shorts) can be mounted, and autoplaying, before the navigation
  // settles, so a rule with pauseMediaSelector pauses whatever matches it first.
  function pauseMedia(rule) {
    if (!rule.pauseMediaSelector) return 0;
    var media = document.querySelectorAll(rule.pauseMediaSelector);
  ...
  var leavingFrom = null; // the path a redirect is already under way from: a slow navigation isn't counted twice
  ...
  function applyRouteRedirects() {
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      // Already on the way out: keep the player quiet while the page changes, without using up the cap.
      if (leavingFrom === path) { pauseMedia(rule); return true; }
      ...
      var paused = pauseMedia(rule);
      leavingFrom = path;
      log(rule.name + ': ' + path + ' -> ' + rule.to + (paused ? ' (paused ' + paused + ' media element(s))' : ''));
  ...
  function install(cfg) {
    ...
    // matters for a redirected route (YouTube Shorts, TikTok For You), where even a short delay would
    // let a player autoplay.
    adPollTimer = setInterval(function () {
      guard('storyAds', applyStoryAds);
      if ((config.routeRedirects || []).length) schedule();
